  ...
  <div class="pb-24">
    <div class="flex items-center justify-between mb-8">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-3xl font-black text-slate-900 tracking-tight">Edit Event</h1>
          <p class="text-slate-500 font-medium mt-1">Update event details.</p>
        </div>
      </div>
    ...
    <div v-if="loading" class="animate-pulse space-y-6">
      <div class="h-[400px] bg-slate-200 rounded-2xl"></div>
    </div>
      ...
      <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-6">
          ...
          <div>
            ...
            <textarea v-model="form.description" rows="5" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium text-sm resize-none"></textarea>
          </div>
          ...
          <div>
            ...
            <input v-model="form.location" type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium text-sm" />
          </div>
        </div>
      </div>
...
const submitForm = async () => {
  ...
  isSaving.value = true;
  try {
    await api.patch(`/events/${route.params.id}`, form.value);
    router.push('/events');
  } catch (error) {
