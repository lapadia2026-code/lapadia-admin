  ...
  <div class="pb-24">
    <div class="flex items-center justify-between mb-8">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-3xl font-black text-slate-900 tracking-tight">Edit Blog Post</h1>
          <p class="text-slate-500 font-medium mt-1">Update your article details.</p>
        </div>
      </div>
    ...
    <div v-if="loading" class="animate-pulse space-y-6">
      <div class="h-[600px] bg-slate-200 rounded-2xl"></div>
    </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <div class="space-y-6">
            <div>
              ...
              <input v-model="form.title" type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium text-lg" />
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          ...
          <div class="space-y-4">
            <div>
              ...
              <div :class="['px-4 py-3 rounded-lg font-bold text-sm text-center border', form.published ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200']">
            ...
            <div>
              ...
              <input v-model="form.authorName" type="text" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium text-sm" />
            </div>

            <div>
              <label class="block text-sm font-bold text-slate-700 mb-2">Tags (comma separated)</label>
              <input v-model="tagsInput" type="text" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium text-sm" />
            </div>
          </div>
        </div>
      </div>
...
onMounted(async () => {
  try {
    ...
    form.value = {
      ...
      authorName: data.authorName || 'Lapadia Team',
      tags: data.tags || [],
...
const submitForm = async () => {
  ...
  prepareForm();
  try {
    await api.patch(`/blogs/${route.params.id}`, form.value);
    router.push('/blogs');
  } catch (error) {
    console.error('Failed to update blog', error);
    alert('Failed to update blog. Please check your connection.');
  } finally {
